                                 ...
                                 int maxY)
{
	...
	for (size_t i = 0; i < balls.size(); ++i)
	{
		...
		if (i != m_grabbedBall)
		{
			...
			{
				if (FRICTION < length(momentumVec))
				{
					ball.velocity -= deltaTime * FRICTION * normalize(momentumVec) / ball.mass;
				}
				else
				{
...
void BallController::onMouseDown(std::vector<Ball>& balls,
                                 float mouseX,
                                 float mouseY)
{
	for (size_t i = 0; i < balls.size(); ++i)
	{
		...
		{
			m_grabbedBall = i; // BW AWARE, if you change the order of the balls in the vector, this becomes invalid!
...
void BallController::onMouseMove(std::vector<Ball>& balls,
                                 float mouseX,
                                 float mouseY)
{
	if (m_grabbedBall != -1)
                                    ...
                                    Ball& b2)
{
	...
	if (collisionDepth > 0)
	{
		// Push away the balls based on ratio of mases 
...
bool BallController::isMouseOnBall(Ball& b,
                                   float mouseX,
                                   float mouseY)
{
