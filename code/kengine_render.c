...
TileRenderWorkThread(void *Data)
{
    ...
    app_render_commands *RenderCommands = Work->RenderCommands;
        ...
        ++SortEntryIndex, ++SortEntry)
    {
        ...
        switch(Header->Type)
        {
            case RenderGroupEntry_render_entry_clear:
            {
                ...
                DrawRectangle(OutputTarget, vMin, vMax, RenderEntry->Color, RenderEntry->Color, ClipRect);
            } break;
            ...
            case RenderGroupEntry_render_entry_bitmap:
            {
                ...
                DrawBitmap(OutputTarget, RenderEntry->P, ScaledXAxis, ScaledYAxis, RenderEntry->Color, RenderEntry->Bitmap, ClipRect);
            } break;
            case RenderGroupEntry_render_entry_rectangle:
            {
                render_entry_rectangle *RenderEntry = (render_entry_rectangle *)EntryData;
                
                DrawRectangle(OutputTarget, RenderEntry->P, V2Add(RenderEntry->P, RenderEntry->Dim), RenderEntry->Color, RenderEntry->AltColor, ClipRect);
            } break;
...
internal void
SoftwareRenderCommands(platform_work_queue *RenderQueue, app_render_commands *RenderCommands, loaded_bitmap *OutputTarget)
{
#define TileCountX 4
#define TileCountY 4
        ...
        ++TileY)
    {
            ...
            ++TileX)
        {
            ...
            Platform.AddWorkEntry(RenderQueue, TileRenderWorkThread, Work);
        }
    }
...
// TODO(kstandbridge): preprocessor to generate swaps?
inline void
SwapTileSortEntries(tile_sort_entry *A, tile_sort_entry *B)
{
...
internal void
SortRenderCommands(app_render_commands *RenderCommands, void *SortMemory)
{
    u32 Count = RenderCommands->PushBufferElementCount;
    tile_sort_entry *Entries = (tile_sort_entry *)(RenderCommands->PushBufferBase + RenderCommands->SortEntryAt);
    
    
    // TODO(kstandbridge): When switching to Merge or Radix sort we will use this memory
    SortMemory;
    
    
    // NOTE(kstandbridge): Bubble sort 0(n^2)
    for(u32 OuterIndex = 0;
        OuterIndex< Count;
        ++OuterIndex)
    {
        ...
        for(u32 InnerIndex = 0;
            ...
            ++InnerIndex)
        {
            tile_sort_entry *EntryA = Entries + InnerIndex;
            tile_sort_entry *EntryB = EntryA + 1;
            
            if(EntryA->SortKey > EntryB->SortKey)
            {
                SwapTileSortEntries(EntryA, EntryB);
                ListIsSorted = false;
            }
        }
